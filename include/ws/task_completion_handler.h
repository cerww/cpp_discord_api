...
struct args_to_tuple_t{
	...
	constexpr auto operator()(args&&... Args)const noexcept(std::make_tuple(std::forward<args>(Args)...)) -> decltype(std::make_tuple(std::forward<args>(Args)...)){
...
namespace boost::asio{	
	...
	public:
		...
		return_type get() { return std::move(m_return_obj); }
	private:
		task<void> m_return_obj;
	};

	template<>
	class async_result<use_task_return_ec_t, void(system::error_code)> {
	public:
		using return_type = task<system::error_code>;
		struct completion_handler_type {
			...
			promise<system::error_code> promise{};
		};
		...
		return_type get() { return std::move(m_return_obj); }
	private:
		task<system::error_code> m_return_obj;
	};
	...
	public:
		...
		using return_type = task<task_return_type>;
		struct completion_handler_type {
			...
			promise<task_return_type> promise{};
			fn f;
		};
		...
		return_type get() { return std::move(m_return_obj); }
	private:
		task<task_return_type> m_return_obj;
	};
}

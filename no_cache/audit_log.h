...
struct audit_log_entry {
	...
	std::optional<additional_entry_info> options;
	std::optional<std::string> reason;

	friend void from_json(const nlohmann::json& json, audit_log_entry& entry) {
		entry.target_id = json.value("target_id", snowflake(0));
		entry.changes = json.value("changes", std::vector<audit_log_change>());
		entry.user_id = json["user_id"].get<snowflake>();
		entry.id = json["id"].get<snowflake>();
		entry.m_action_type = json["action_type"].get<audit_log_entry_type>();
		entry.options = json.value("options", std::optional<additional_entry_info>());
		entry.reason = json.value("reason", std::optional<std::string>());
	}
};
...
struct audit_log {
	...
	std::vector<partial_integration> integrations;
	std::vector<audit_log_entry> audit_log_entries;

	friend void from_json(const nlohmann::json& json, audit_log& logs) {
		logs.webhooks = json["webhooks"].get<std::vector<webhook>>();
		logs.users = json["users"].get<std::vector<user>>();
		logs.integrations = json["integrations"].get<std::vector<partial_integration>>();
		logs.audit_log_entries = json["audit_log_entries"].get<std::vector<audit_log_entry>>();
	}
};
}
